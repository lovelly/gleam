...
import (
	...
	"github.com/lovelly/gleam/sql/util/charset"
	"github.com/lovelly/gleam/sql/util/hack"
	"github.com/lovelly/gleam/sql/util/stringutil"
...
func builtinConvert(args []types.Datum, _ context.Context) (d types.Datum, err error) {
	...
	if err != nil {
		log.Printf("Convert %s to %s with error: %v", str, Charset, err)
		return d, errors.Trace(err)
	}
	d.SetString(target)
	return d, nil
...
func builtinChar(args []types.Datum, ctx context.Context) (d types.Datum, err error) {
	...
	if !argCharset.IsNull() {
		...
		if err != nil {
			log.Printf("Convert %s to %s with error: %v", resultStr, char, err)
			return d, errors.Trace(err)
		}
	}
	d.SetString(resultStr)
